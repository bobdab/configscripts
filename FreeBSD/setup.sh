...
`"
	...
	echo "======================================================================"
	echo "You might want to include the root user ID and a regular user ID here:"
        read -p "Do you want to configure a user ID? (y/n) " yn_add_user
        while [ -z "${usr_id}" ]; do
...
if [ "${yn}" = "y" ]; then
    ...
    if [ ! -d "${src_dir}/common" ]; then
        ...
        # Refresh the configscripts package
	echo "========================================================================="
	echo "Warning: if curl is not installed, I will install it as a binary package."
	echo "If you want to install it as a port, kill this script now, install"
	echo "curl as a port, then rerun this script."
	pkg install curl

        if [ -f master.tar ]; then
            echo "You already have Bob's .tar configscripts file."
            read -p "Do you want to download a fresh copy? (y/n)" yn_temp
        ...
        src_dir=$(realpath ./configscripts-master/FreeBSD)
    fi

    echo "The source directory for FreeBSD scripts is ${src_dir}."
    ...
    if [ "${dm}" = "twm" ]; then
        ...
        cp -i "${src_dir}/twm/xinitrc" /usr/local/etc/X11/xinit
        cp -i "${src_dir}/common/.vimrc" "${usr_home}/.vimrc"
    fi

    if [ "${dm}" = "fluxbox" ]; then
        # src_dir is the directory from which this script runs
        chown "${usr_id}" "${src_dir}/fluxbox/xtermgo.sh" 
        chown "${usr_id}" "${src_dir}/fluxbox/.profile"
        cp "${src_dir}/fluxbox/xtermgo.sh" /usr/local/etc/X11/xinit
        cp "${src_dir}/fluxbox/.profile" "${usr_home}"
    fi

    cp -i "${src_dir}/common/privoxyconfig" "${usr_home}/privoxyconfig"
    cp -i "${src_dir}/common/rc.conf.example" "${usr_home}/rc.conf.example"
    # - - -  seamonkey prefs
    if [ ! -d "${usr_home}/${usr_id}/FirefoxProfiles" ]; then

	    cp "${src_dir}/common/FirefoxProfilesAug2015.tgz" "${usr_home}/"
	    cd "${usr_home}"
	    tar -xf "FirefoxProfilesAug2015.tgz"
      if [ "${usr_id}" = "root" ]; then
          tar -xf "${src_dir}/common/mozilla_prefs_root.tgz"
      else
          tar -xf "${src_dir}/common/mozilla_prefs_super.tgz"
      		# fix the profile.ini to point to this users' Firefox profile
			    cat "${usr_home}/.mozilla/seamonkey/profiles.ini" | \
						sed -e "s/super/${usr_id}/" > /tmp/newprofile.ini
      fi
      cp /tmp/newprofile.ini "${usr_home}/.mozilla/seamonkey/"
    else
    	echo "Skipping copy of FirefoxProfiles directory."
    fi
		chown -R "${usr_id}":"${usr_id}" "${usr_home}"
    cd "${src_dir}"
    src_prefs="${src_dir}/common/seamonkey-prefs.js"

    prefs=$(find ${usr_home}/.mozilla -name 'prefs[.]js')
    echo "flist is ${prefs}"
...
for f in $(echo "${prefs}"); do
    # I previously kept several prefs....js files as different
    # profiles, now I use the ~/FirefoxProrfiles directory.
    echo "Top of for-loop: ${f}"
    chown "${usr_id}:${usr_id}" "${f}"
    d=$(dirname "${f}")
    echo "Note: Copying mozilla/firefox/seamonkey prefs from ${f} to ${d}"
    cp "${f}" "${d}/prefs.js"
